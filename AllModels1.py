...
if __name__ == '__main__':
    ...
    for compnay in companies:
        ...
        svr_lin_yhat = svr_lin_model.predict(X_test)
        ...
        history = model.fit(XX_train2, YY_train1, epochs=100, batch_size=72, validation_data=(XX_test2, YY_test1),
                            verbose=4, shuffle=False)
        ...
        plt.plot(dates, svr_yhat, color = 'xkcd:mauve', label = 'SVR')
        #plt.plot(dates, svr_lin_yhat, color = 'xkcd:leaf green', label = 'Linear SVR')
        #plt.plot(dates, knn_yhat, color = 'xkcd:wine', label = 'K-Nearest Neighbor')
        ...
        mse_svr = mse(y_test, svr_yhat)
        rmse_svr = rmse(y_test, svr_yhat)
        mape_svr = mape(y_test, svr_yhat)

        print(mse_svr, rmse_svr, mape_svr)

        mse_lin_svr = mse(y_test, svr_lin_yhat)
        rmse_lin_svr = rmse(y_test, svr_lin_yhat)
        mape_lin_svr = mape(y_test, svr_lin_yhat)
        ...
        #### bar charts with emotions

        Emotions = []
        Subjectivity = []

        grouped_test_set = test_set.groupby('Date')

        for date, tgroup in grouped_test_set:

            positive, negative, neutral = 0, 0, 0
            subj, obj = 0, 0

            n  = len(tgroup)
            for _, tgrow in tgroup.iterrows():

                emotion = tgrow['compound_emotion']
                sbj = tgrow['subjectivity_score']
            ...
            Emotions.append({'Date': date, 'Positive': positive, 'Negative': negative, 'Neutral': neutral})
            Subjectivity.append({'Date': date, 'Subjective': subj, 'Objective': obj})


        Emotions1 = pd.DataFrame(Emotions)
        Subjectivity1 = pd.DataFrame(Subjectivity)

        dates1 = Emotions1['Date']
        ...
        rect1 = axs.bar(x - width/2, Emotions1['Positive'], width, label = 'Positive', color='m')
        rect2 = axs.bar(x+ width/2, Emotions1['Neutral'], width, label='Neutral', color ='b')
        rect3 = axs.bar(x + width/2, Emotions1['Negative'], width, label = 'Negative', color = 'g')

        axs.set_xticks(x, dates1, fontsize = '8')
        ...
        plt.show()
        fig.savefig(output_dir + 'Emotions.png')
        plt.close()
        ...
        rect1 = axs.bar(x - width / 2, Subjectivity1['Subjective'], width, label='Subjective', color='m')
        rect3 = axs.bar(x + width / 2, Subjectivity1['Objective'], width, label='Objective', color='g')
        ...
        plt.show()
        fig.savefig(output_dir + 'Subjectivity.png')
        plt.close()
